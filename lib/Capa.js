...
class Capa {
    ...
    getInfo() {
        console.log("enviando opciones", this.opciones);
        return {
            ...
            tipo:this.tipo,
            icono:this.icono,
            decimales:this.opciones.decimales !== undefined?this.opciones.decimales:2,
            menuEstaciones:this.opciones.menuEstaciones?true:false,
...
class CapaVectorial extends Capa {
    constructor(proveedor, codigo, nombre, origen, opciones, grupos, icono) {
        let opcionesDefault = {
            temporal:false,
            formatos:{geoJSON:false},
            opacidad:100
        }
        if (!opciones) opciones = {};
        super(proveedor, codigo, nombre, origen, "vectorial", extend(true, opcionesDefault, opciones), grupos, icono);
    }
    getInfo() {
        let info = super.getInfo();
        info.formatos = this.opciones.formatos;
        info.temporal = this.opciones.temporal;
        if (this.opciones.estilos) {
            info.estilos = this.opciones.estilos.toString()
        }
